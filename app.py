...
import pandas as pd
import json
import ast  # Tambahan untuk membaca kutip tunggal AI
...
if analyze_btn:
    ...
    try:
        ...
        try:
            ...
            # ─── PERBAIKAN 1: BISA MEMBACA KUTIP TUNGGAL ───
            try:
                parsed = json.loads(clean_json)
            except json.JSONDecodeError:
                # Jika json.loads gagal karena kutip tunggal, gunakan ast
...
if st.session_state.analysis_done:
    ...
    # NARRATIVE BOX AI
    st.markdown(f'<div class="narrative-box {verdict_class}"><div style="font-size: 0.75rem; font-weight: 800; text-transform: uppercase; letter-spacing: 0.1em; opacity: 0.7; margin-bottom: 0.5rem;">Keputusan Agent</div><h3>{icon} {res.get("title", "Analisis Selesai")}</h3><p>{res.get("narasi", "")}</p></div>', unsafe_allow_html=True)

    hemat_str = "Lebih Hemat" if selisih > 0 else "Lebih Mahal"
    st.markdown(f'<div class="metric-grid"><div class="modern-metric"><div class="metric-label">BBM / Bulan</div><div class="metric-val" style="color: #111827;">Rp {biaya_bbm:,}</div></div><div class="modern-metric"><div class="metric-label">Listrik EV / Bulan</div><div class="metric-val" style="color: #059669;">Rp {biaya_ev:,}</div></div><div class="modern-metric" style="background: {"#f0fdf4" if selisih > 0 else "#fef2f2"}; border-color: {"#d1fae5" if selisih > 0 else "#fee2e2"};"><div class="metric-label" style="color: {"#065f46" if selisih > 0 else "#991b1b"}">Penghematan</div><div class="metric-val" style="color: {"#059669" if selisih > 0 else "#dc2626"};">Rp {abs(selisih):,}</div></div><div class="modern-metric modern-metric-dark"><div class="metric-label" style="color: #9ca3af;">Reduksi CO2 / Tahun</div><div class="metric-val">{co2_kg:,.0f} <span style="font-size: 1rem; color: #6b7280;">Kg</span></div></div></div>', unsafe_allow_html=True)
    ...
    with col_right:
        st.markdown('<div class="subsidy-box" style="margin-bottom: 1.5rem;"><div class="subsidy-title">🏛️ Insentif Pemerintah</div><div class="subsidy-item"><span>✓</span> Potongan PPN dari 11% menjadi 1%</div><div class="subsidy-item"><span>✓</span> Bebas Ganjil-Genap di DKI Jakarta</div><div class="subsidy-item"><span>✓</span> Subsidi Insentif Langsung s.d Rp 7 Juta</div></div>', unsafe_allow_html=True)
        ...
        ev_html = f'<div class="content-box"><div class="box-title">🚗 Opsi Unit EV Terdekat</div><div class="ev-card"><div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;"><div><h4 style="margin: 0; font-weight: 800; color: #111827; font-size: 1.05rem;">{nama_ev}</h4><p style="margin: 0; font-size: 0.8rem; font-weight: bold; color: #059669; margin-top: 4px;">{spec_ev}</p></div><div class="ev-price">{harga_ev_txt}</div></div><p style="margin: 0; font-size: 0.82rem; color: #64748b; font-weight: 500; line-height: 1.4;">Rekomendasi disesuaikan otomatis dengan batas anggaran Rp {investasi_awal_juta:,.0f} Juta rupiah.</p></div></div>'
    ...
    res_title = res.get('title', 'Selesai dianalisis')
    jarak_km = st.session_state.user_data.get("jarak_harian_km", 0)
    
    laporan_html = f'<div style="margin-top: 1.5rem;" class="content-box"><div class="box-title">📑 Laporan Analisis Mendalam</div><h4 style="color: #065f46; font-size: 1rem; margin-top: 1rem; margin-bottom: 0.5rem; font-weight: 800;">💰 ANALISIS BIAYA</h4><ul style="color: #374151; font-weight: 500; font-size: 0.95rem; line-height: 1.6; margin-bottom: 1rem;"><li><b>Biaya BBM Bulanan:</b> Rp {biaya_bbm:,}/bulan</li><li><b>Estimasi Listrik EV Bulanan:</b> Rp {biaya_ev:,}/bulan</li><li><b>Potensi Penghematan:</b> Berselisih <b>Rp {abs(selisih):,}/bulan</b> ({hemat_str}).</li></ul><h4 style="color: #065f46; font-size: 1rem; margin-top: 1rem; margin-bottom: 0.5rem; font-weight: 800;">🌿 DAMPAK LINGKUNGAN</h4><ul style="color: #374151; font-weight: 500; font-size: 0.95rem; line-height: 1.6; margin-bottom: 1rem;"><li>Memotong emisi karbon sebesar <b>{ton_co2} Ton CO₂/tahun</b>.</li><li>Setara dengan menanam <b>{pohon_setara} pohon</b> per tahun.</li></ul><h4 style="color: #065f46; font-size: 1rem; margin-top: 1rem; margin-bottom: 0.5rem; font-weight: 800;">🚗 REKOMENDASI KENDARAAN</h4><ul style="color: #374151; font-weight: 500; font-size: 0.95rem; line-height: 1.6; margin-bottom: 1rem;"><li>Sesuai budget Rp {investasi_awal_juta:,.0f} Juta dan jarak harian {jarak_km} km, sistem memetakan opsi terbaik di panel samping.</li></ul><h4 style="color: #065f46; font-size: 1rem; margin-top: 1rem; margin-bottom: 0.5rem; font-weight: 800;">📊 TITIK BALIK MODAL (BEP)</h4><ul style="color: #374151; font-weight: 500; font-size: 0.95rem; line-height: 1.6; margin-bottom: 1rem;"><li><b>Investasi Bersih:</b> Rp {investasi_awal_juta:,.0f} Juta (setelah potong trade-in).</li><li>Proyeksi perpotongan modal dapat dipantau di grafik area.</li></ul><h4 style="color: #065f46; font-size: 1rem; margin-top: 1rem; margin-bottom: 0.5rem; font-weight: 800;">✅ KESIMPULAN</h4><ul style="color: #374151; font-weight: 500; font-size: 0.95rem; line-height: 1.6; margin-bottom: 0;"><li>Status Keputusan: <b>{res_title}</b>.</li><li><i>Dasar perhitungan menggunakan tarif resmi ESDM, Pertamina, dan PLN yang berlaku.</i></li></ul></div>'
    st.markdown(laporan_html, unsafe_allow_html=True)


    # ─── PERBAIKAN 2: MENAMPILKAN CHAT AI ───
    st.markdown('<div style="margin-top: 3rem;" class="chat-container"><div class="box-title">💬 Diskusi Lanjut dengan Agent</div>', unsafe_allow_html=True)
    
    is_first_bot_msg_skipped = False
    for msg in st.session_state.conversation[1:]:
        if msg["role"] == "user":
            # Hapus teks "Catatan sistem rahasia" agar tidak muncul di layar user
            tampil_user = msg["content"].split("\n\n(Catatan sistem rahasia:")[0]
            st.markdown(f'<div class="chat-user-msg">{tampil_user}</div>', unsafe_allow_html=True)
        elif msg["role"] == "assistant":
            # Jangan tampilkan chat asisten yang pertama (karena itu isinya JSON)
            if not is_first_bot_msg_skipped:
                is_first_bot_msg_skipped = True
                continue
            
            # Tampilkan pesan asisten yang kedua dan seterusnya (Jawaban Chat)
    ...
    if submitted and user_input:
        ...
        try:
            ...
            # ─── PERBAIKAN 3: SUNTIKAN RAHASIA ANTI-JSON ───
            prompt_chat_aman = user_input + "\n\n(Catatan sistem rahasia: Jawab pertanyaan saya di atas menggunakan paragraf biasa yang enak dibaca. DILARANG menggunakan format JSON!)"
            
            response, updated_history = run_agent(prompt_chat_aman, st.session_state.conversation.copy(), status_callback=update_thinking_chat)
            # ───────────────────────────────────────────────
            
            chat_think_placeholder.empty() 
            
            # Bersihkan catatan rahasia dari history agar tidak dilihat user di kemudian waktu
            updated_history[-2]["content"] = user_input
